...
newpattern<-secondsccresult[,5]
snp5<-c()
for (j in 1:length(newpattern))
{
snp5[j]=variablelist[[newpattern[j]]]
}
secondsccresult[,5]<-unlist(snp5)
write.table(secondsccresult,file = "./result/transformedantsccresult5.csv",sep = ",",row.names = FALSE,col.names=TRUE)  # write
