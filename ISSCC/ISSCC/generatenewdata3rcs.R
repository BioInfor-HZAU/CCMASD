setwd("./aisscc-test")
print("Please select the cs source data:") 
sccsamples<-read.csv(file.choose(),header=F,skip=1)  # choose the source data，注意一定要选最开始的源数据
...
#写入文件
write.table(sccsamples,file = "./result/newdata3rcs.csv",sep = ",",row.names = FALSE,col.names=TRUE)  # write the newdata to file
